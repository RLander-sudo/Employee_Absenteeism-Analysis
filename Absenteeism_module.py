...
# Creating a special class that we are going to use to predict new data
class absenteeism_model():
    ...
    def __init__(self, model_file, scaler_file):
        ...
        # Takeing the data file (CSV) and preprocess it in the same way as in the "Absenteeism_Logistic_Regression" file
    def load_and_clean_data(self, data_file):

        #importing the data
        df = pd.read_csv(data_file, delimiter = ',')
        # storing the data in a new variable for later use
        self.df_with_predictions = df.copy()
        # droping the 'ID' column
        df = df.drop(['ID'], axis = 1)
        # to preserve the code in the previous file, adding a column with 'NaN' strings
        df['Absenteeism Time in Hours'] = 'NaN'

        # create a seperate dataframe, containing dummy values for LL available reasons
